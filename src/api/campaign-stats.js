...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const camp = await conn.query(`
      SELECT Id, Name, IsActive, StartDate, EndDate${quotaField ? ','+quotaField : ''}
      FROM Campaign
      WHERE Id='${campaignId}'
      LIMIT 1
    `);
    ...
    if (countsFormula) {
      const q = await conn.query(`
        ...
        WHERE CampaignId='${campaignId}' AND Counts_Toward_Quota__c = true
      `);
      used = q.totalSize || 0;
    } else {
      // fallback: gunakan HasResponded = TRUE
      const q = await conn.query(`
        SELECT COUNT() FROM CampaignMember
