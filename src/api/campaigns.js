...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const category = String(req.query.category || '').trim();          // 'all' atau salah satu picklist
    ...
    } // status 'all' = tidak tambah kondisi waktu
    ...
    // filter kategori jika ada dan field tersedia
    ...
    // order by (tetap pakai StartDate DESC NULLS LAST; tidak ada filter "terbaru" di UI)
    ...
    const records = (dataRes.records || []).map(r => ({
      id: r.Id, name: r.Name, status: r.Status, type: r.Type, isActive: r.IsActive,
      startDate: r.StartDate, endDate: r.EndDate, description: r.Description, createdDate: r.CreatedDate,
      imageUrl: r.Promo_Image_URL__c || null, price: r.Price__c ?? null, discountPercent: r.Discount_Percent__c ?? null,
      category: r.Category__c || null, landingUrl: r.Landing_URL__c || null, priority: r.Web_Priority__c ?? null,
