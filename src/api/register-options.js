...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      if (type === 'campuses' || type === 'campus') {
        ...
        try {
          ...
          const r2 = await conn2.query(`
            SELECT Master_School_c, Master_School_r.Name
            FROM Account
          ...
          (r2.records || []).forEach(x => {
            ...
            const nm = x.Master_School_r && x.Master_School_r.Name;
      ...
      // ---------- SEKOLAH (autocomplete): type=sekolah, param t|term ----------
      if (type === 'sekolah') {
        const conn = await login();
      ...
      if (type === 'schools' || type === 'school') {
        ...
        // Try 1: MasterSchool_c (Name/NPSN_c)
        try {
